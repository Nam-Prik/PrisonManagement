...
export const visitmentController = {
  ...
  async getById(c: Context) {
    ...
    if (!data)
      return c.json({ error: 'Not Found', message: 'Visitment not found', statusCode: 404 }, 404)
    return c.json<ApiResponse<typeof data>>({ data, message: 'Success' })
  },
  ...
  async lookupPrisoner(c: Context) {
    ...
    if (!code)
      return c.json({ error: 'Bad Request', message: 'Code is required', statusCode: 400 }, 400)
  ...
  async lookupPerson(c: Context) {
    ...
    if (!id)
      return c.json({ error: 'Bad Request', message: 'ID is required', statusCode: 400 }, 400)
